...
public class MyVoucherActivity extends BaseActivity {
    ...
    private VoucherDataAdapter jAdapter;
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        jAdapter = new VoucherDataAdapter();
        ...
        jAdapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                CouponDataBean.DataBean item = jAdapter.getItem(position);
                //数据是使用Intent返回
    ...
    public void getData(int pageNum) {
        ...
        OkGo.<String>get(URLs.MY_COUPON).tag(this).params("user_id", userId)
            ...
            public void onSuccess(Response<String> response) {
                ...
                Gson gson = new Gson();
                CouponDataBean couponDataBean = gson.fromJson(response.body(), CouponDataBean.class);
                onResultSuccess(couponDataBean);
            }
    ...
    private void onResultSuccess(Object response) {
        if (!isLoadMore) {
            jPageNum++;
            CouponDataBean couponDataBean = (CouponDataBean) response;

            if (null == response || null == couponDataBean.getData()) {
                showEmpty();
            }
            fillData(couponDataBean.getData());
        } else {
            jPageNum++;
            CouponDataBean couponDataBean = (CouponDataBean) response;
            loadMoreSuccess(couponDataBean.getData());
        }
    }

    private void fillData(List<CouponDataBean.DataBean> listData) {
        if (listData.size() == 0) {
            showEmpty();
        } else {
            if (isRefresh)
    ...
    private void showData(List<CouponDataBean.DataBean> dataList) {
        if (isOnDestroy)
            return;
        multipleStatusView.showOutContentView(jSwipeRefreshLayout);
    ...
    private void loadMoreSuccess(List<CouponDataBean.DataBean> dataList) {
        jSwipeRefreshLayout.finishLoadMore();
